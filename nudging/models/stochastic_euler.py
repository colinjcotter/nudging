...
class Euler_SD(base_model):
    ...
    def setup(self, comm=MPI.COMM_WORLD):
        ...
        Dt = self.dt
        if self.salt:
            psi_mod = self.psi0*Dt+self.noise_scale*self.dU_1*Dt**0.5  # SALT noise
        else:
            psi_mod = self.psi0*Dt

        #print(type(psi_mod))
        self.psi_mod = psi_mod
        #print(type(self.psi_mod))
        ...
        if not self.salt:
            a_int += Dt*p*self.noise_scale*self.dU_3*Dt**0.5*dx
    ...
    def run(self, X0, X1):
        ...
        for step in range(self.nsteps):
            ...
            self.psi_solver.solve()
            self.phi_fn.project(self.dt*self.psi0)
            self.phi_mod_fn.project(self.psi0*self.dt+self.noise_scale*self.dU_3*self.dt**0.5)
